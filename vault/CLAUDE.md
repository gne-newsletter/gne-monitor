...
**참고**: `analyze.py summarize`는 기본값이 `--since 2025`라 2024년 이전 논문은 `--all-years` 없이는 영구히 누락됨. 2026-07-07에 이 문제로 2024년 논문 7건이 통째로 미분석 상태였던 걸 발견·수정. **앞으로 정기 실행 시 `--all-years` 사용을 기본으로 할 것.**
...
## 기획기사(feature article) — 콘텐츠 전략 신설 (2026-07-07)

논문 목록의 "환자에게 의미"(1~2문장 자동 생성)와는 별개로, 매달 논문 1편(또는 관련 기술 트렌드)을 깊이 있게 풀어내는 독립 콘텐츠를 뉴스레터에 추가.

**취지**: 환우들이 "치료제가 언제 나오나"라는 수동적 관심에 머물러 있고 병 자체에 대한 객관적 지식이 부족한 상태 — 논문을 단순 번역이 아니라 병의 맥락(이전 연구와의 연결, 증명된 것/한계)까지 포함해 서술해 환자가 스스로 치료 방향성을 판단할 기준을 갖게 하는 것이 목표.

**범위 확장**: GNE 논문에 국한하지 않고, AI로 인한 개인화 의료 패러다임 전환 트렌드(예: AI 기반 전신 스캐너, 신약재창출 등)도 방향성 기사로 포함 — 희귀질환이 개인화 의료의 최대 수혜 분야가 될 가능성이 높아 환우회가 미리 안목을 갖추도록 준비.

**우선순위 및 상세 근거**: Claude memory `project_newsletter_workflow.md` 참조 (1순위 3건, 2순위 3건 확정, 2026-07-07 기준).
...
### 2026-07-07 (논문 전체 분석 완료 + 기획기사 콘텐츠 전략 신설)

- **`analyze.py summarize` 연도 제한 버그 발견·수정**
  - 기본값 `--since 2025`로 인해 2024년 논문 7건이 영구 미분석 상태였음
  - `--all-years` 옵션으로 미분석 11건 중 10건 처리 (1건은 abstract 없어 불가)
  - 결과: 관련도 ≥8점 논문 24건 → 32건

- **index.html 논문 페이지 갱신**
  - KPI: 총 수집 논문 35→38, AI 요약 완료 10→37
  - "AI 고관련도 논문" 테이블 5행 → 32행 전체 반영
  - 8점 논문의 "환자에게 의미" 칸은 설계상(9점 이상만 생성) 비워둔 채 유지하기로 결정 — 억지로 채우지 않음

- **기획기사(feature article) 콘텐츠 전략 신설** (상세는 위 섹션 참조)

- **7월호 뉴스레터**: "논문 1건 + 정책 1건" 간소화 발행으로 결정, `newsletter_draft_202607.md` 작성 (검토 대기)
